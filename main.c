...
RasenganEffect InitRasengan(Vector3 position, Color startColor, Color endColor,
                            float coreRadius, float innerRadius, float middleRadius,
                            float outerRadius)
{
...
void UpdateRasenganOrbiters(RasenganEffect *effect)
{
    for (int i = 0; i < NUM_ORBITERS; i++)
    {
        ...
        Vector3 pos = {
            relativePos.x + effect->position.x,
...
void DrawRasengan(const RasenganEffect *effect)
{
    ...
    for (int i = 0; i < NUM_ORBITERS; i++)
    {
        ...
        int count = (orb->trail[TRAIL_SIZE - 1].x == 0 &&
                     orb->trail[TRAIL_SIZE - 1].y == 0 &&
                     orb->trail[TRAIL_SIZE - 1].z == 0)
                        ? orb->trailIndex
                        : TRAIL_SIZE;
        ...
        if (count > 1)
        {
            ...
            for (int j = 1; j < count; j++)
            {
                ...
                if (prevPoint.x == effect->position.x && prevPoint.y == effect->position.y && prevPoint.z == effect->position.z)
                {
...
int main(void)
{
    ...
    RasenganEffect rasengan1 = InitRasengan(
